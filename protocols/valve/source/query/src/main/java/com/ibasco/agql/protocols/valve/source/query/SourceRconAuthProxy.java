...
public final class SourceRconAuthProxy implements Closeable {
    ...
    private static final RetryPolicy<Channel> ACQUIRE_RETRY_POLICY = RetryPolicy.<Channel>builder().handle(ConnectTimeoutException.class, ConnectException.class)
                                                                                ...
                                                                                .onRetry(event -> log.debug("AUTH (ACQUIRE) => Failed to acquire channel. Retrying (Attempts: {}, Last Failure: {})", event.getAttemptCount(), event.getLastFailure() != null ? event.getLastFailure().getClass().getSimpleName() : "N/A"))
    ...
    private final AcquireTaskSupplier acquireTaskSupplier = new AcquireTaskSupplier();

    private final ThreadLocal<FailsafeExecutor<RconParcel>> authExecutors = ThreadLocal.withInitial(() -> {
        FailsafeExecutor<RconParcel> e = Failsafe.with(AUTH_CHANNEL_POLICY);
        log.debug("AUTH => Instantiating new auth executor for thread: {} (Executor: {})", Thread.currentThread().getName(), e);
        return e;
    });

    private final BiFunction<SourceRconResponse, Channel, Channel> RCON_AUTH_RESPONSE_HANDLER = (response, channel) -> {
    ...
    private CompletableFuture<Channel> authenticate(Channel channel, Credentials credentials) {
        ...
        return messenger.send(authRequest, channel).thenCombine(CompletableFuture.completedFuture(channel), RCON_AUTH_RESPONSE_HANDLER);
    }
    ...
    private RconParcel authResponseHandler(RconParcel parcel) {
        ...
        if (parcel.hasError()) {
            Throwable cause;
            if (parcel.error() instanceof CompletionException) {
                cause = parcel.error().getCause();
                cause.printStackTrace();
                //if the server forcibly closed the connection but we didn't receive any response from it,
                //then at this point it can mean:
    ...
    private CompletableFuture<RconParcel> sendCmdRequest(final RconParcel parcel) {
        ...
        if (!isAuthenticated(address))
            return CompletableFuture.completedFuture(parcel.error(new RconNotYetAuthException(String.format("Address '%s' not yet authenticated. Make sure to call authenticate().", channel.remoteAddress()), SourceRconAuthReason.NOT_AUTHENTICATED)));
        //is the channel authenticated already?
        if (isAuthenticated(channel))
            return sendParcel(parcel);
        log.debug("{} AUTH => Channel not yet authenticated. Attempting to authenticate with server", NettyUtil.id(channel));
    ...
    private SourceRconResponse release(RconParcel parcel, Throwable error) {
        ...
        try {
            if (parcel.hasError()) {
                if (parcel.error() instanceof CompletionException)
                    throw (CompletionException) parcel.error();
                throw new CompletionException(parcel.error());
            } else {
    ...
    //2) wrap channeel and envelope into a parcel
    ...
    private CompletableFuture<RconParcel> createParcel(final InetSocketAddress address, SourceRconRequest request) {
        ...
        return acquire(envelope) //acquire channel
                                 .thenCombine(CompletableFuture.completedFuture(envelope), RconParcel::new) //wrap acquired channel and envelope in parcel
    ...
    private CompletableFuture<Channel> acquire(final Envelope<SourceRconRequest> envelope) {
        ...
        return acquireExecutor.getStageAsync(acquireTaskSupplier.envelope(envelope)).thenCompose(this::register).handle(channelStatistics);
    }
    ...
    private class AcquireTaskSupplier implements ContextualSupplier<Channel, CompletableFuture<Channel>> {

        private final Deque<Envelope<SourceRconRequest>> taskQueue = new ConcurrentLinkedDeque<>();

        private AcquireTaskSupplier() {}

        private AcquireTaskSupplier envelope(Envelope<SourceRconRequest> envelope) {
            taskQueue.addFirst(envelope);
            return this;
        }

        @Override
        public CompletableFuture<Channel> get(ExecutionContext<Channel> context) throws Throwable {
            Envelope<SourceRconRequest> envelope = taskQueue.pollFirst();
            if (envelope == null)
                return ConcurrentUtil.failedFuture(new IllegalStateException("No envelope available"));
    ...
    private static class RconHelper {
        ...
        public RconParcel validate(RconParcel parcel) {
            ...
            Objects.requireNonNull(envelope.recipient(), "Destination address is missing in envelope");
            Objects.requireNonNull(channel, "Channel must not be null");
        ...
        public Channel validate(Channel channel) {
