        ...
        try {
            ...
            if (request instanceof SourceRconAuthRequest) {
                ...
                if (authRequest.getPassword() == null) {
                    ...
                    if (credentials == null)
                        throw new RconNotYetAuthException(String.format(NOT_YET_AUTH_MSG, address), authRequest, address, SourceRconAuthReason.NOT_AUTHENTICATED);
                    //make sure the credentials are still valid
                    if (!credentials.isValid())
                        throw new RconInvalidCredentialsException(String.format(INVALID_CREDENTIALS_MSG, address), authRequest, address, SourceRconAuthReason.INVALIDATED);
                    //Update credentials
                ...
                if (context.properties().authenticated()) {
                    ...
                    if (credentials == null)
                        throw new RconNotYetAuthException(String.format(NOT_YET_AUTH_MSG, address), request, address, SourceRconAuthReason.NOT_AUTHENTICATED);
                    //Are the credentials still valid?
                    if (!credentials.isValid())
                        throw new RconInvalidCredentialsException(String.format(INVALID_CREDENTIALS_MSG, address), request, address, SourceRconAuthReason.INVALIDATED);
                    ...
                    if (!reauthenticate)
                        throw new RconNotYetAuthException(String.format(NOT_YET_AUTH_MSG, address), request, address, SourceRconAuthReason.INVALIDATED);
            ...
            if (e instanceof RconException)
                throw e;
            //Wrap caught exceptions with RconException
            throw new RconException(e, context.properties().request(), address);
        }
        return future;
    }
    ...
    private SourceRconChannelContext processAuthResponse(final SourceRconChannelContext context) {
        ...
        if (response.isAuthenticated()) {
            ...
            if (credentialsPresent && credentialsValid) {
                ...
                throw new RconInvalidCredentialsException(String.format(INVALID_CREDENTIALS_MSG, address), context.properties().request(), address, SourceRconAuthReason.INVALIDATED);
            }
        }
        return context;
    }
